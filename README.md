...
# 🍶 오늘 주막(Today Jumak) - 막걸리 & 안주 AI 추천 서비스
...
- **AI 맞춤형 큐레이션 (주모 페르소나 & 4단계 추천 포맷)**
  ...
  - 사극 주모 톤(~했소, ~구려, ~추천하겠소)을 유지하며 일관된 4단계 문단 구조로 답변을 생성합니다.
- **섹션 이동 네비게이션 바 (Sticky Nav)** 🆕
  - 상단에 고정된 메뉴바를 통해 '주막 소개', 'AI 추천받기', '비밀 장부' 등 3개의 주요 페이지/섹션으로 부드럽게(Smooth Scroll) 이동할 수 있어 사용자 편의성을 높였습니다.
- **직관적이고 반응형인 웹 디자인**
  - 바닐라 HTML/CSS/JS로 구현되었으며, 모바일, 태블릿, 데스크톱 어디서든 자연스러운 반응형(Responsive) 카드 UI를 제공합니다.
- **프론트엔드 상태 관리 (로딩/성공/실패 처리)** 🆕
  - JavaScript의 `fetch` API를 활용하여 비동기 통신의 3가지 상태를 명확히 처리했습니다.
  - **로딩 (Loading)**: 요청 시작 시 버튼 텍스트를 "주모가 막걸리 찾는 중... ⏳"으로 변경하고, 결과창을 띄워 대기 상태를 시각적으로 안내합니다.
  - **성공 (Success)**: 백엔드에서 정상적으로 JSON 응답을 받으면(`.then`), 결과창(`innerHTML`)에 AI의 답변을 렌더링하고 버튼을 원래 상태로 복구합니다.
  - **실패 (Failure)**: 네트워크 오류나 서버 에러 발생 시(`.catch`), "아이고, 주막에 불이 났소!"라는 친근한 에러 메시지를 띄워 사용자 경험(UX)이 저하되지 않도록 예외 처리했습니다.
- **견고한 에러 핸들링 및 UX 개선**
...
#### 1. `api/ai_jumo.py` : 핵심 비즈니스 로직 전담
- **역할:** 사용자의 입력(지역, 맛)을 받아 프롬프트를 구성하고, LLM(OpenAI API)과 통신하여 맞춤형 추천 결과를 반환하는 **핵심 기능**을 담당합니다.
- **설계 이점 (명확성):** Vercel 서버리스 환경에서는 파일명이 곧 API 엔드포인트가 됩니다. 프론트엔드에서 `fetch('/api/ai_jumo')`로 호출하게 함으로써, 해당 API가 '주모 AI 기능'을 수행한다는 것을 직관적으로 알 수 있게 설계했습니다.

#### 2. `api/index.py` : 기본 진입점 및 헬스 체크 (Health Check)
- **역할:** `/api/` 경로로 접근했을 때 기본적으로 응답하는 파일로, 서버가 정상적으로 작동하고 있는지 상태를 확인하는 용도로 구성했습니다.
- **설계 이점 (확장성):** 모든 코드를 하나의 파일(예: `app.py`)에 몰아넣으면 코드가 방대해지고(Spaghetti Code) 유지보수가 어려워집니다. `index.py`를 기본 안내데스크로 두고 기능을 분리해 두었기 때문에, 향후 `ai_anju.py`(안주 전용 추천), `save_db.py`(장부 DB 저장) 등 **새로운 기능을 추가할 때 기존 코드를 건드리지 않고 유연하게 확장**할 수 있습니다.
