...
- **AI 맞춤형 큐레이션 (주모 페르소나 & 4단계 추천 포맷)**
  - 단순한 랜덤 추천이 아닌, 사용자가 입력한 지역과 선택한 맛 키워드(🍯 달달함, 🍋 새콤함, ⚡ 톡 쏘는 탄산, 🌰 담백함, 🎯 아무거나 좋아요)를 프롬프트로 조합합니다.
...
- **견고한 입력 검증 및 UX 개선**
  - **지역 입력 유효성 검증 (오타 및 비정상 입력 방지)** 🆕: 영문(예: `vjd`), 숫자, 특수기호, 자모 단독 입력 시 즉시 주막 모달 팝업으로 한글 2글자 이상의 올바른 지역명 입력을 유도하여 엉뚱한 AI 추천이 발생하는 오류를 원천 차단했습니다.
